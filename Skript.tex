...
\begin{document}
    ...
    \begin{satz} % Satz 9
        \marginnote{[07. Feb]}
        Zu $\sigma$-endlichen Maßen $\mu_j$ auf $\mA_j$, $j=1, \ldots, n$ existiert genau ein Produktmaß $\pi$ auf $\pair{\mA_1\otimes\cdots\otimes \mA_n}$ mit
        \begin{align*}
            \pi\of{A_1\times\cdots \times} &= \prod_{j=1}^{n} \mu_j\of{A_j}
        \end{align*}

        \begin{proof}
            Sei $\mE_j \coloneqq \mA_j$ als Erzeuger. Nach der dem Eindeutigkeitssatz für Maße gibt es also höchstens ein solches Produktmaß. Um auch die Existenz zu beweisen, gehen wir induktiv vor. Wir setzen
            \begin{align*}
                \pi_l &\coloneqq \mu_1 \otimes \cdots \otimes \mu_l\quad 2\leq l \leq n\\
                \impl \pi_2 &= \mu_1\otimes\mu_2\quad\text{auf } \mA_1 \otimes \mA_2\\
                \impl \pi_3 &= \pi_2 \otimes \mu_3\quad\text{auf }\pair{\mA_1\otimes\mA_2}\otimes\mA_3\\
                &= \pair{\mu_1\otimes\mu_2}\otimes\mu_3
                \intertext{Wir könnten $\pi_3$ aber auch definieren als}
                \pi_3' &= \mu_1\otimes\pair{\mu_2\otimes\mu_3}
                \intertext{Nach dem Eindeutigkeitssatz muss aber gelten}
                \pi_3 &= \pi_3'
            \end{align*}
            Wenn man dieses Argument iteriert, folgt die Assoziativität des Produktmaßes im Allgemeinen. Das heißt, da wir die Existenz des Produktmaß zweier Maße bereits gezeigt haben, ergibt sich induktiv die Behauptung.\qedhere
        \end{proof}
    \end{satz}
    ...
    \section{[*] Integrale bezüglich Bildmaß und Jacobi Transformationsformel}

    \subsection{Integrale bezüglich Bildmaß}
    \thispagestyle{pagenumberonly}

    Wir haben $\pair{X, \mA}$, $\pair{X', \mA'}$ Messräume und $T: X\to X'$ eine $\mA-\mA'$-messbare Funktion sowie $\mu$ ein Maß auf $\pair{X, \mA}$. Dann ist $T\of{\mu}$ definiert durch $T\of{\mu}\of{A'} \coloneqq \mu\of{T^{-1}\of{A'}}$ für $A'\in\mA'$. Wie hängt das Integrieren bezüglich Maß und Bildmaß zusammen?

    \begin{satz} % Satz 1
        Seien $\pair{X, \mA}$, $\pair{X', \mA'}$ Messräume und $T: X\to X'$ eine $\mA-\mA'$-messbare Funktion, $\mu$ ein Maß auf $\pair{X, \mA}$ sowie $f: X' \to \overline{\R}$ eine Funktion. Dann gilt: Ist $f$ bezüglich $T\of{\mu}$ integrierbar, so ist $f\circ T$ bezüglich $\mu$ integrierbar und es gilt
        \begin{align*}
            \int_{X'}^{} f\dif T\of{\mu} &= \int_{X}^{} f\circ T \dif \mu\tag{1}
        \end{align*}
        Ist $f\geq 0$, so gilt (1) immer.

        \begin{proof}
            Mit algebraischer Induktion. (Siehe Übung).
        \end{proof}
    \end{satz}

    \subsection{Jacobi-Transformationsformel}

    Situation: Seien $X, Y\subseteq\R^d$ offen und $\Phi: X \to Y$ ein Diffeomorphismus (d.h. bijektiv, stetig differenzierbar und $\Phi^{-1}$ auch stetig differenzierbar)

    \begin{satz}[Jacobi] % Satz 3
        Sei $\Phi: X \to Y$ mit $X, Y\subseteq\R^d$ offen ein Diffeomorphismus. Dann gilt
        \begin{enumerate}[label=(\roman*)]
            \item
            Für $A\in\mB^d \cap X$
            \begin{align*}
                \lambda^d\of{\Phi\of{A}} &= \int_{A}^{} \abs{\det \Dif\Phi} \dif\lambda^d
            \end{align*}
            \item Für alle $f: X \to \interv{0, \infty}$ numerischen, meßbaren Funktionen
            \begin{align*}
                \int_{Y}^{} f \dif \lambda^d &= \int_{X}^{} f\circ \Phi \abs{\det \Dif\Phi} \dif \lambda^d\\
                &= \int_{X}^{} f\of{\Phi\of{x}}\abs{\det \Dif\Phi\of{x}} \lambda^d\of{\dif x}
            \end{align*}
            \item $f: Y \to\overline{\R}$ ist genau dann über $Y$ Lebesgue-integrierbar, wenn $f\circ \Phi \abs{\det \Dif\Phi}$ über $X$ Lebegue-integrierbar ist und es gilt
            \begin{align*}
                \int_{X}^{} f \dif\lambda^d &= \int_{X}^{} f\circ\Phi\abs{\det\Dif\Phi} \dif \lambda^d
            \end{align*}
        \end{enumerate}
        ...
        \begin{proof}
            \theoremescape
            \begin{enumerate}[label=(\roman*)]
                \item Es reicht zu zeigen, dass $\forall f\in\mM_{*}\of{Y, \mB^d \cap Y}$ ist
                \begin{align*}
                    \int_{Y}^{} f \dif \lambda^d &\leq \int_{X}^{} f\circ\Phi\abs{\det\Dif\Phi} \dif \lambda^d\tag{2}
                    \intertext{Haben wir nämlich die Ungleichung (2), definieren wir $\Psi \coloneqq \Phi^{-1}$, $h\coloneqq f\circ\Phi \abs{\det\Dif \Phi \circ \Psi}$, dann folgt mit (2)}
                    \impl \int_{X}^{} h \dif \lambda^d &\leq \int_{Y}^{} h\circ \Psi\abs{\det \Dif\Psi} \dif \lambda^d\\
                    \intertext{Außerdem ist}
                    \Dif\Psi\of{y} &= \Dif\of{\Phi^{-1}}\of{y} = \pair{\Dif \Phi\of{\Psi\of{y}}}^{-1} = \interv{\Dif \Phi \circ \Psi}^{-1}\\
                    \impl \det \Dif\Psi\of{y} &= \frac{1}{\det\Dif\Phi\of{\Psi\of{y}}}\\
                    \intertext{Gemeinsam folgt dann}
                    \int_{X}^{} f\circ\Phi\abs{\det\Dif\Phi} \dif \lambda^d &\leq~??\\
                    &\leq \int_{Y}^{} h\circ \Psi\abs{\det\Dif \Psi} \dif \lambda^d\\
                    &= \int_{Y}^{} f\underbrace{\abs{\det \Dif \Phi \circ \Psi}\abs{\det \Dif\Psi}}_{=1} \dif \lambda^d = \int_{Y}^{} f\dif\lambda^d
                    \intertext{Das heißt aus (2) erhalten wir auch die Ungleichung in die andere Richtung und damit Gleichheit. Wir müssen nun noch (2) beweisen: ???}
                \end{align*}
            \end{enumerate}
        \end{proof}
    \end{satz}
    ...
    \textbf{Schritt 1.} Wir betrachten den $\R^d$ und eine lineare Abbildung $T: \R^k \to\R^d$. Außerdem sei $L \coloneqq T\of{\R^k} = k$-dimensionaler Unterraum des $\R^d$. Wenn wir dann $V \subseteq \R^k$ offen betrachten, dann ist $T\of{V}$ relativ-offen in $L \subseteq \R^d$.

    \begin{definition}[Isometrische Matrizen]
        Wir definieren
        \begin{align*}
            \mO\of{k, d} &= \set{G\in\mL\of{\R^k, \R^d}: G^{\mathrm{T}}G = \charfunc_{\R^k}}
            \intertext{als die Menge der isometrischen Matrizen. Dann gilt bezüglich des Standardskalaprodukts}
            \scalprod{Gx, Gx}_{\R^d} &= \scalprod{x, G^{\mathrm{T}}Gx}_{\R^d} = \scalprod{x, x}_{\R^d}
        \end{align*}
    \end{definition}

    \begin{beobachtung}[Polarzerlegung]
        Sei $T\in\mL\of{\R^k, \R^d}$ mit vollem Rang (d.h. $T$ hat Rang $k$). Dann existieren $Q\in\mO\of{k, d}$ und $S\in\mL\of{\R^k, \R^k}$ mit $S$ symmetrisch und positiv-definit, sodass $T = QS$. Ferner gilt dann
        \begin{align*}
            S^2 &= T^{\mathrm{T}}T\\
            \det S &= \sqrt{\det\of{T^{\mathrm{T}}T}}
        \end{align*}

        \begin{proof}
            $T^{\mathrm{T}}T: \R^k \to \R^k$ ist linear und $T^{\mathrm{T}}T \neq 0$ für $\R^k \ni x \neq 0$, denn
            \begin{align*}
                \scalprod{x, T^{\mathrm{T}}Tx} &= \scalprod{Tx, Tx}\geq 0
                \intertext{Das heißt $T^{\mathrm{T}}T$ ist positiv-definit und symmetrisch, weil}
                \pair{T^{\mathrm{T}}T}^{\mathrm{T}} &= T^{\mathrm{T}}T
                \intertext{Das heißt es gibt ein $R\in\mathcal{SO}$, sodass}
                T^{\mathrm{T}}T &= R^{\mathrm{T}}DR
                \intertext{wobei}
                D &= \diag\of{d_1, \ldots, d_k}\quad d_j \geq 0~\forall j=1, \ldots, k\\
                \sqrt{D} &= \diag\of{\sqrt{d_1}, \ldots, \sqrt{d_k}}\\
                \impl S &= R^{\mathrm{T}}\sqrt{D}R\\
                \impl S^{\mathrm{T}} = S\\
                S^2 &= R^{\mathrm{T}}\sqrt{D}RR^{\mathrm{T}}\sqrt{D}R = R^{\mathrm{T}}DR = T^{\mathrm{T}}T
                \intertext{Wir definieren}
                Q &\coloneqq TS^{-1}\\
                S^{-1} &= \pair{R^{\mathrm{T}}\sqrt{D}R}^{-1} = R^{-1}\sqrt{D}^{-1}\pair{R^{\mathrm{T}}}{-1}\\
                &= R^{\mathrm{T}}\sqrt{D}^{-1}R = R^{\mathrm{T}}\diag\pair{d_1^{-1}, \ldots, d_n^{-1}}^{-1}R\\
                \impl T &= QS\\
                Q^{\mathrm{T}}Q &= \pair{TS^{-1}}^{\mathrm{T}}\pair{TS^{-1}} = S^{-1}T^{\mathrm{T}}TS^{-1} = S^{-1}S^2 S^{-1} = \charfunc_{\R^k}\\
                \pair{\det S}^2 &= \det S^2 = \det T^{\mathrm T}T\\
                \impl \det S &= \sqrt{\det T^{\mathrm{T}}T}\qedhere
            \end{align*}
        \end{proof}
        \noindent Sei $A\subseteq \R^k$ offen. Dann sei $H^k\of{T\of{A}}$ das $k$-dimensionale Volumen von $T\of{A}$ als Teilmenge von $L\coloneqq T\of{\R^d}$. Es gilt
        \begin{align*}
            H^{k}\of{T\of{A}} &= H^k\of{Q\of{S\of{A}}} = H^k\of{S\of{A}} \coloneqq \abs{\det S} \lambda^k\of{A}
        \end{align*}
    \end{beobachtung}

    \begin{definition}
        Sei $V\subseteq \R^k$ offen und $\varphi: V\to\R^d$ eine Immersion. Das heißt
        \begin{align*}
            \forall x\in V\colon D_{\varphi}\of{x}: \R^k \to\R^d\text{ hat vollen Rang}
        \end{align*}
        Wir definieren
        \begin{align*}
            \mA_{\varphi} \coloneqq \set{B\in\varphi\of{V}: \varphi ???}
        \end{align*}
        ??
    \end{definition}
    ...
    \begin{lemma}
        \theoremescape
        \begin{enumerate}[label=(\roman*)]
            \item $\mA_{\varphi}$ ist eine $\sigma$-Algebra
            \item ??
            \item $\pair{\varphi\of{V}, \mA_{\varphi}, H^k_{\varphi}}$ ist ein Maßraum.
        \end{enumerate}
    \end{lemma}

    \begin{lemma} % Lemma 4
        $\pair{D_{\varphi}\of{x}}^{\mathrm{T}}D_{\varphi}\of{x}$ ist eine $k\times k$-Matrix. Die Matrix ist gegeben durch
        \begin{align*}
            \pair{\pair{D_{\varphi}}^{\mathrm{T}}D_{\varphi}}_{l, m} &= \scalprod{\partial_l \varphi, \partial_m \varphi}
        \end{align*}
    \end{lemma}

    \noindent\textbf{Schritt 2}: Integration über $k$-dimensionale Untermannigfaltigkeiten des $\R^d$. Sei $M \subseteq \R^d$ mit $M\neq \emptyset$.

    \begin{definition}[Karte]
        $\pair{\varphi, U}$ nennen wir eine Karte, wenn $U\subseteq\R^k$ offen ist und $\varphi: U \to M$ ein Homomorphismus ist. $M$ ist eine $k$-dimensionale Untermannigfaltigkeit, wenn
        \begin{align*}
            \forall p\in M\ex R>0, \text{ Karte}\pair{\varphi, k}: B_{R}\of{p}\cap M \subseteq \varphi\of{U}
        \end{align*}
    \end{definition}

    \begin{definition}[Atlas]
        $\pair{\varphi_j, U_j}_{j\in I}$ $I$ ist endlich oder abzählbar $\bigcup_{i\in I} \varphi_i\of{U_k} = M$
    \end{definition}

    \begin{beobachtung}
        Jeder Punkt in $M$ liegt in endlich vielen $\varphi_j\of{U_j}$.
    \end{beobachtung}

    \marginnote{[14. Feb]}
    \noindent\textbf{Schritt 3}: Regulärer Rand offener Gebiete, offene Gebiete mit $\mC^1$-Rand.

    \begin{definition}
        Sei $\Omega\in\R^d$ offen und $\partial \Omega$ Rand $= \overline{\Omega}\cap\overline{\pair{\comp{\Omega}}}$. Wir definieren
        \begin{align*}
            \partial_{\text{reg}}\Omega &= \set{x\in\partial\Omega: \exists p > 0, \exists\text{Funktion} G\in\mC^{1}\of{B_p\of{x}}, \forall G\of{x}\neq 0, \Omega\cap B_p\of{x} = G^{-1}\of{\pair{-\infty, 0}}}
        \end{align*}
        als den regulären Rand von $\Omega$. Hat $\Omega$ $\mC^1$-Rand, haben $\partial\Omega = \partial_{\text{reg}}\Omega$.
    \end{definition}

    \begin{lemma}
        Sei $x\in\partial_{\text{reg}}\Omega$. Dann folgt $\Omega \cap B_p\of{x} = G^{-1}\of{\set{0}}$.

        \begin{proof}
            Ist $G\of{x} < 0$. Dann folgt für $r> 0$, dass $\forall y\in B_{r}\of{x}: G\of{y} < 0$. Ist $G\of{x} > 0$, so folgt für $r' > 0$, dass $\exists\in B_{r'}\of{x}: G\of{y} > 0$. Daraus folgt $x\not\in\partial\Omega$.\\
            Ist $G\of{x} = 0$. $a\in\R^d$
            \begin{align*}
                \partial_t G\of{x+ ta} &= \Dif G\of{x+ta}\interv{a} = \scalprod{\nabla G\of{x+ta}, a} \to_{t\to 0} \scalprod{\partial G\of{x}, a} \neq 0
            \end{align*}
            Beobachte: Satz von der impliziten Funktion. (+Tangentialraum) (?)\qedhere
        \end{proof}
    \end{lemma}

    \begin{lemma}
        Sei $\Omega$ offen. Dann existiert für alle $x\in\partial_{\text{reg}}\Omega$ existiert genau ein Normalenvektor $V\of{x}$, sodass
        \begin{enumerate}[label=(\roman*)]
            \item $\abs{V\of{x}} = 1$
            \item $V\of{x} = \partial{T_x\partial \Omega}^{\mathrm{T}}$ (wobei $T$ der Tangentialraum ist), das heißt $V\of{x}$ ist parallel zu $\nabla G\of{x}$
            \item $\forall x\in\partial_{\text{reg}}\Omega\ex p > 0: x+tV\of{x} \in\Omega~\forall -f < t < 0$, $x+ tV\of{x}\in\comp{\Omega}~\forall 0 < t < f$.
        \end{enumerate}
        \begin{proof}
            Definiere
            \begin{align*}
                V\of{x} &\coloneqq \frac{\nabla G\of{x}}{\abs{\nabla G\of{x}}}
            \end{align*}
            und rechne Eigenschaften nach.\qedhere
        \end{proof}
    \end{lemma}
    ...
    \begin{satz}[Satz von Gauß]
        \label{satz:von-gauss}
        Sei $\Omega$ offen und beschränkt mit $\mC^1$-Rand. Sei $F\in\mC^{1}\of{\Omega, \R^d} \cap \mC\of{\overline{\Omega}, \R^d}$ mit $\fdiv F = \sum_{j=1}^{n} \partial_{x_j} F_j \subseteq \mL^1\of{\Omega}$. Dann gilt
        \begin{align*}
            \int_{\Omega}^{} \fdiv F \dif \lambda^d &= \int_{\partial\Omega}^{} F\nu \dif H_{\partial \Omega}^{d-1}
        \end{align*}
        mit $\nu$ als äußere Normale von $\partial \Omega$ im Punkt $x\in\partial\Omega$.
    \end{satz}

    \begin{korollar}
        \begin{align*}
            \int_{\Omega}^{} \partial_j f \dif \lambda^d &= \int_{\partial\Omega}^{} f\nu_j \dif H_{\partial\Omega}^{d-1}
        \end{align*}
    \end{korollar}

    \begin{lemma}[Integral unter dem Graphen einer Funktion (mit Rand)]
        Sei $V\subseteq\R^{d-1}$ offen und $g\in\mC^{1}\of{V}$. $A=\set{x=\pair{y,t}: t\leq g\of{y}}$, $F\in\mC^{1}\of{A, \R^{d}}\cap\mC^{\overline{A}, \R^d}$. Es gelte $K \subseteq V \times \R$ kompakt, sodass $F = 0$ auf $A \setminus K$. Dann folgt
        \begin{align*}
            \int_{A}^{} \fdiv F \dif \lambda^d &= \int_{M}^{} ?
        \end{align*}
        wobei $M = $ graph$\of{G}$.
    \end{lemma}

    \begin{proof}[Beweis von Satz~\ref{satz:von-gauss}]
        \textit{(Fehlt hier)}
    \end{proof}
